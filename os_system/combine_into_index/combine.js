console.log("Moving to modular system");
process.exit(1);
const fs = require("fs");
...
if (!args.values["no-increment"] && !args.values["only-modules"]) fs.writeFileSync(__dirname + "/boot/00-pcos.js", pcosHeader);
...
function createMediaStructure(currentlyScanning = __dirname, notFirstStep = false) {
	...
	if (!notFirstStep) readdir = readdir.filter(a => a != "apps" && a != "boot" && a != "modules" && a != "combine.js");
	let structure = {};
...
if (args.values["only-modules"]) process.exit(0);
