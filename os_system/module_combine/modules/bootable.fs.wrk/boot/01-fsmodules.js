async function loadModules() {
	...
	async function loadKHRL() {
		let khrlFiles = await modules.fs.ls("system/etc/keys/khrl");
		for (let khrlFile of khrlFiles) {
			let khrl = JSON.parse(await modules.fs.read("system/etc/keys/khrl/" + khrlFile));
	...
	async function recursiveKeyVerify(key, khrl) {
		...
		if (key.keyInfo.signedBy) {
			signedByKey = JSON.parse(await modules.fs.read("system/etc/keys/" + key.keyInfo.signedBy));
	...
	try {
		let moduleList = (await modules.fs.ls(modules.defaultSystem + "/modules")).sort(function(a, b) {
			if (a == "00-keys.fs") return -1; // Ensuring keys are loaded first
			if (b == "00-keys.fs") return 1;
		...
		for (let moduleName of moduleList) {
			...
			if (moduleName == "00-keys.fs") {
				try {
					...
					if (modules.core.bootMode != "disable-harden") {
						panic("KEYS_MODULE_VERIFICATION_FAILED", {
				...
				try {
					let signingKey = JSON.parse(await modules.fs.read("system/etc/keys/" + fullModuleFile.buildInfo.signer));
					...
					if (critical && modules.core.bootMode != "disable-harden") {
						panic("CRITICAL_MODULE_VERIFICATION_FAILED", {
			...
			if (moduleName == "00-keys.fs") {
				modules.fs.mounts.system = await modules.mounts.overlayMount({
					mounts: [ modules.defaultSystem, mountpoint ],
					read_only: true
				});
				await loadKHRL();
			}
		...
		await panic("MODULE_SYSTEM_FAILURE", {
			underlyingJS: e
		});
	}
}
